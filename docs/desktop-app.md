...
- `失焦自动隐藏`：默认开启，启动器失焦后自动隐藏；拖动窗口和打开 DevTools 时不会立刻隐藏。
- `完成提示音`：控制会话完成通知是否静音。
...
- 范围切换：`全部`、`Codex`、`Claude`、`Trae`，可用 `⌘1` 到 `⌘4` 切换。
...
- Trae 当前没有 CLI resume，点击会提示无法恢复；用 `⌘↵` 或行内完整视图按钮查看。
- 行内快捷操作包含：`⭐` 置顶/取消置顶、复制恢复命令、打开完整视图。
- `⌘↵` 打开完整视图；`⌘/` 打开快捷键说明；`Esc` 清空搜索或关闭快捷键说明。

## 完整查看器

`/` 是完整的只读查看器。它按来源标签页显示 `Codex`、`Claude Code`、`Trae`，再按项目分组。项目标题可以展开/收起；有项目路径的分组会提供项目范围搜索。
...
- `图库` 会扫描最近会话里的 inline 图片，支持 `全部` / `Codex` / `Claude` / `Trae` 筛选。
...
- 统计可按 `全部` / `Codex` / `Claude` / `Trae` 过滤。
- Codex 配额来自本地 Codex CLI session tail 中的 `rate_limits` 快照；Claude Code 和 Trae 没有对应的本地配额文件。
...
- Trae 会话支持查看、导出、Gist 和发布分享，但不支持 Orca resume。
...
| `/api/activity` | `GET` | 扫描本地 Codex / Claude / Trae 会话，生成 26 周活跃度、小时分布和项目统计。 | local-only |
