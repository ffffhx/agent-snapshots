...
async function writePersistedSessionImages(session, entries) {
    ...
    try {
      ...
      ON CONFLICT(cache_key) DO UPDATE SET
        ...
        updated_at=excluded.updated_at`).run(cacheKey, filePath, sessionMtimeKey(session), Number(session?.mtimeMs || new Date(session?.mtime || 0).getTime() || 0), sessionSize(session), sessionRef(session), normalizeSource(session?.engine || "codex"), normalizeSource(session?.engine || "codex"), JSON.stringify((entries || []).map(cacheableImageEntry)), Array.isArray(entries) ? entries.length : 0, Date.now());
    }
    catch {
