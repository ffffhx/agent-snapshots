...
function updateRowStatOnly(db, row, candidate) {
    ...
    WHERE cache_key = ?`).run(summary.mtime || isoTime(cachedCandidate.mtimeMs), numberTime(summary.mtime) || Number(cachedCandidate.mtimeMs || 0), Number(summary.size || cachedCandidate.size || 0), summary.filePath || cachedCandidate.filePath || "", Number(cachedCandidate.mtimeMs || 0), Number(cachedCandidate.size || cachedCandidate.sizeBytes || 0), state.complete ? 1 : 0, state.live ? 1 : 0, JSON.stringify(summary), JSON.stringify(cachedCandidate), Date.now(), row.cache_key);
