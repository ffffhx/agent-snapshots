...
export function isGitHubGistPublishError(error) {
    return error instanceof GitHubGistPublishError;
}
export async function createGitHubGist(html, { publicGist = false } = {}) {
...
export async function createGitHubGistFromFile(filePath, { publicGist = false } = {}) {
    ...
    if (!gistUrl || !gistId) {
        throw new GitHubGistPublishError("gist_publish_failed", "gh gist create 未返回可识别的 Gist 地址，请确认 GitHub CLI 可用并重试。");
    }
    return {
        gistUrl,
...
function formatGitHubGistError(error) {
    ...
    if (/not logged in|authentication|auth login|gh auth/i.test(message)) {
        return new GitHubGistPublishError("gh_not_authenticated", "GitHub CLI 未登录或认证失败。" + hint + (message ? "\n" + message : ""));
    }
    if (isNetworkFailureMessage(message)) {
        return new GitHubGistPublishError("network_failure", "创建 GitHub Gist 失败。" + hint + (message ? "\n" + message : ""));
    }
    return new GitHubGistPublishError("gist_publish_failed", "创建 GitHub Gist 失败。" + hint + (message ? "\n" + message : ""));
}
