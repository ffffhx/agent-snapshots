...
export async function syncSearchIndex({ codexHome, claudeHome, traeHome, traeAppHome, traeRecordingsDir, source = "all", cwd = "", includeArchived = true, completeOnly = true, scanLimit = 1200, updateLimit = 60, includeTools = true, includeToolOutput = false, withTokens = true, }) {
    ...
    for (const summary of sessions) {
        ...
        try {
            ...
            upsert.run(ref, summary.engine || "codex", engineKey(summary.engine), summary.title || "", summary.cwd || "", summary.displayCwd || summary.cwd || "", mtime, fold, JSON.stringify(doc.fields), JSON.stringify(doc.segments), JSON.stringify(summary), tokens?.total || 0, tokens?.input || 0, tokens?.output || 0, tokens?.model || "", Date.now());
            updated += 1;
            indexed += 1;
        }
        catch {
