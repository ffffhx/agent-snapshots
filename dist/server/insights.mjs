...
export async function buildInsights({ codexHome, claudeHome, traeHome, traeAppHome, traeRecordingsDir, listSessions, loadSnapshot, source = "all", limit = DEFAULT_SCAN_LIMIT, }) {
...
function displayToolName(name) {
    ...
    if (lower === "web_search" || lower === "websearch")
        return "WebSearch";
    if (isShellToolName(raw))
        return "Bash";
    if (lower === "multi_edit" || lower === "multiedit")
        return "MultiEdit";
    if (lower === "read_file")
        return "Read";
    if (lower === "edit_file")
...
async function writePersistedInsights(scopeKey, watermark, scanLimit, result) {
    try {
      ...
      ON CONFLICT(cache_key) DO UPDATE SET
        ...
        created_at=excluded.created_at`).run(key, scopeKey, Number(watermark || 0), Number(scanLimit || 0), JSON.stringify(result), Date.now());
