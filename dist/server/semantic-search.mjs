...
export async function semanticSearchSnapshot(snapshot, { query, limit = DEFAULT_RESULT_LIMIT, model = defaultEmbeddingModel(), baseUrl = defaultOllamaBaseUrl(), }) {
...
export function buildSearchChunks(snapshot, { maxChunks = MAX_SEARCH_CHUNKS } = {}) {
    const chunks = [];
...
function pushTurnChunks(chunks, turns, sourceLabel, maxChunks) {
...
function splitForEmbedding(text) {
...
function positiveTurnNumber(value) {
...
function documentCacheKey(snapshot, chunks, model, baseUrl) {
...
async function getDocumentEmbeddings(cacheKey, chunks, options) {
