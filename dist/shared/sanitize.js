...
const SLASH_COMMAND_PATTERN = /<command-name>([\s\S]*?)<\/command-name>\s*(?:<command-message>[\s\S]*?<\/command-message>\s*)?(?:<command-args>([\s\S]*?)<\/command-args>\s*)?/gi;
...
function formatSlashCommand(name, args) {
...
function unwrapSubagentNotification(inner) {
...
function collectStatusText(status) {
    ...
    const stringValues = Object.values(record).filter((value) => typeof value === "string");
